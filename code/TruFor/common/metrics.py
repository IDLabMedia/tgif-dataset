...
def computeMetrics_mod(values, gt, th):
    values = values > th
    values = values.flatten().astype(np.uint8)
    gt = gt.flatten().astype(np.uint8)
...
def computeF1_mod(FP, TP, FN, TN):
    if TP == 0 and FN == 0 and FP == 0:
        return 1
    return 2 * TP / np.maximum((2 * TP + FN + FP), 1e-32)

def computeLocF1_th(map, gt):
    FP, TP, FN, TN = computeMetrics_mod(map, gt, 0.5)
    f1 = computeF1_mod(FP, TP, FN, TN)
