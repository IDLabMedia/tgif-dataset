...
class ManipulationDataset(Dataset):
    def __init__(self, path, image_size, train=False, delimiter = ' '):
        self.train = train
        self.path = path
        ...
        with open(self.path, 'r') as f:
            lines = f.readlines()
            for line in lines:
                parts = line.rstrip().split(self.delimiter)
                try:
...
class MixDataset(Dataset):
    def __init__(self, paths, image_size, train=False, class_weight=None):
        ...
        self.dataset_list = []
        for path in paths:
            self.dataset_list.append(ManipulationDataset(path, image_size, train=train,  delimiter=' '))
