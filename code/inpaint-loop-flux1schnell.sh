...
inpainting_model_name="flux1schnell"
guidance_scale=0 # flux1schnell guidance scale should be 0
guidance_scale_cmd="--guidance_scale_min $guidance_scale --guidance_scale_max $guidance_scale"
# flux1schnell can generate high-quality in only 1 to 4 steps. In practice, I often notice very bad results (like something went wrong) for 1 or 2 steps. So use 3
num_inference_steps_min=3
num_inference_steps_max=6
num_inference_steps_cmd="--num_inference_steps_min $num_inference_steps_min --num_inference_steps_max $num_inference_steps_max"
...
dont_load_cmd=
#dont_load_cmd="--dont_load_inpainting_model"

stop_on_error_cmd=
#stop_on_error_cmd=--stop_on_exception

#nima_itm_cmd=
nima_itm_cmd="--do_nima --do_giqa --do_itm"
...
python3 inpaint-loop.py \
    ...
    --verbose "$VERBOSE" $num_inference_steps_cmd $guidance_scale_cmd \
    --num_images_per_prompt "$NUM_IMAGES_PER_PROMPT" $nima_itm_cmd $skip_cmd $dont_load_cmd $fix_seed_cmd $stop_on_error_cmd \
